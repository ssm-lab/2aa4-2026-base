Python visualization script ```light_visualizer.py``` for rendering a Catan board from JSON files.
...
- (1) Single-render mode
   - Usage: ```python light_visualizer.py base_map.json <state.json>```
   - Renders one board image from the given state JSON file.
- (2) Watch mode
   - Usage: ```python light_visualizer.py base_map.json --watch```
